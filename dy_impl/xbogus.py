...
class XBogus:
    ...
    def build(self, url: str) -> Tuple[str, str, str]:
        ua_md5_array = self._md5_str_to_array(
            self._md5(base64.b64encode(self._rc4_encrypt(self._ua_key, self._user_agent.encode("ISO-8859-1"))).decode("ISO-8859-1"))
        )
        ...
        garbled = self._encoding_conversion2(
            2,
            255,
            self._rc4_encrypt("每".encode("utf-8"), self._encoding_conversion(*merged).encode("ISO-8859-1")).decode("ISO-8859-1"),
        )
        xb = ""
        idx = 0
